...
export default function Products() {
  ...
  const filteredProducts = products.filter(product => {
    ...
    const matchesAisle = aisleFilter === 'all' || product.aisle === aisleFilter;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingProduct) {
        await db.updateProduct(editingProduct.prodid, formData);
      } else {
  ...
  const getAisleColor = (aisle: string) => {
    switch (aisle) {
      case 'Gear': return 'bg-blue-100 text-blue-800 dark:bg-blue-900 dark:text-blue-300';
      case 'Clothes': return 'bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-300';
      case 'Consumables': return 'bg-orange-100 text-orange-800 dark:bg-orange-900 dark:text-orange-300';
  ...
  const getStockStatus = (stock: number) => {
    ...
    if (stock < 20) return { label: 'Low Stock', color: 'bg-yellow-100 text-yellow-800 dark:bg-yellow-900 dark:text-yellow-300' };
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-gray-600 dark:text-gray-400 mt-1">Manage gym products and inventory</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6">
          <div className="flex items-center">
            ...
            <div className="ml-4">
              <p className="text-sm font-medium text-gray-600 dark:text-gray-400">Gear Items</p>
              <p className="text-2xl font-bold text-gray-900 dark:text-white">
                {products.filter(p => p.aisle === 'Gear').length}
              </p>
            </div>
          </div>
        </div>
          ...
          <table className="w-full">
            ...
            <tbody className="divide-y divide-gray-200 dark:divide-gray-600">
              {filteredProducts.map((product) => {
                ...
                return (
                  <tr key={product.prodid} className="hover:bg-gray-50 dark:hover:bg-gray-700">
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div>
                        <div className="text-sm font-medium text-gray-900 dark:text-white">{product.name}</div>
                        <div className="text-sm text-gray-500 dark:text-gray-400">ID: {product.prodid}</div>
                      </div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${getAisleColor(product.aisle)}`}>
                        {product.aisle}
                      </span>
                    </td>
